...
const usePaymentAlert = (user) => {
  ...
  const checkSubscriptionStatus = () => {
    ...
    const paymentType = subscription.paymentMode == 'automatic' ? 'automatic' : 'manual';
...
useEffect(() => {
    if (user && user.subscription) {
      // Check subscription status, but only set isOpen=true if we should show the alert
      const checkAndSetAlert = () => {
        // Store current alert state
        const currentState = { ...alertState };
        
        // Check status (this potentially sets a new alert state)
        checkSubscriptionStatus();
        
        // Now check if we should show this alert today
        setAlertState(prevState => {
          // If there's a new alert type that differs from what we started with
          if (prevState.type && prevState.type !== currentState.type) {
            // Only show it if we haven't shown it today
            const show = shouldShowAlert(prevState.type);
            return { ...prevState, isOpen: show };
          }
          return prevState;
        });
      };
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible' && user && user.subscription) {
        const currentAlertType = alertState.type;
        if (currentAlertType && !shouldShowAlert(currentAlertType)) {
          return; // Don't check again if we've already shown this alert today
        }
        checkSubscriptionStatus();
      }
    };
